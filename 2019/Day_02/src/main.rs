...
impl IntCodeProgram {
    ...
    /// Determine the value of index one & two that produce a specifc output.
    pub fn brute_force_output(&mut self, desired_output: usize) -> usize {
        let init_mem = self.memory.clone();

        /* Brute force the combination that produces the output. */
        for noun in 0..100 {
            for verb in 0..100 {
                /* Reset the program memory and pointer. */
                self.memory = init_mem.clone();
                self.pntr = 0;

                /* Initialise the values in memory. */
                self.memory[1] = noun;
                self.memory[2] = verb;

                /* Has a working combination been found. */
                if self.run(false) == desired_output {
                    return 100 * noun + verb;
                }
            }
        }
        panic!(
            "No noun and verb combination could not be found to reach: '{}'",
            desired_output
        );
    }
}
...
fn main() {
    println!(
        ...
        IntCodeProgram::from_file("./data/input_0.txt").brute_force_output(19690720)
    );
}
