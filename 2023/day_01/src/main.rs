...
fn calibration_sum(input_file: &str, spelled_digits: bool) -> u32 {
    ...
    return ex_nums.iter().sum::<u32>();
}

fn main() {
    println!(
        "Part 1 : Sum of numbers = {}",
        calibration_sum("../data/input.txt", false)
    );

    println!(
        "Part 2 : Sum of numbers = {}",
        calibration_sum("../data/input.txt", true)
    );
}
