 ...
 *
 * PART 1:  Treating your puzzle input as a string of ASCII characters, what is
...
impl KnotHash {
    pub fn new(twist_file: &str, max_num: u32, ascii: bool) -> Self {
        let twists = if ascii {
            let mut tmp: Vec<usize> = read(twist_file)
                .unwrap()
                .into_iter()
                .map(|x| x as usize)
                .collect();

            /* If the file is not empty remove the final char. */
            if tmp.len() > 0 {
...
fn main() {
    println!(
        "Part 1 = {}\nPart 2 = {}",
        KnotHash::new("./data/input.txt", 255, false).verify_round_hash(),
        KnotHash::new("./data/input.txt", 255, true).digest()
    );
}
