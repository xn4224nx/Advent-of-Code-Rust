...
impl ProgramVillage {
    ...
    /// Count the total number of non-connected groups in the village
    pub fn total_groups(&self) -> usize {
        let mut seen_progs: HashSet<u16> = HashSet::new();
        let mut grp_cnt = 0;

        for prog in self.links.keys() {
            if !seen_progs.contains(prog) {
                seen_progs.extend(&self.full_prog_group(*prog));
                grp_cnt += 1;
            }
        }
        return grp_cnt;
...
fn main() {
    println!(
        ...
        ProgramVillage::new("./data/input.txt")
            .full_prog_group(0)
            .len(),
        ProgramVillage::new("./data/input.txt").total_groups()
    );
}
