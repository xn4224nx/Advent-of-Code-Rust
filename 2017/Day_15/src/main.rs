 ...
 *
 * PART 2:  After 5 million pairs, but using this new generator logic, what is
...
impl DualGenerator {
    ...
    pub fn count_next_matches(&mut self, n_nums: usize) -> usize {
        let mut matches = 0;
    ...
    /// For the next n numbers count the non-sequential matches that the
    /// generators produce.
    pub fn non_seq_matches(&mut self, n_nums: usize) -> usize {
        let mut a_vals = Vec::new();
        let mut b_vals = Vec::new();

        /* Generate values till the required number of comparisons can be made. */
        while a_vals.len() < n_nums || b_vals.len() < n_nums {
            self.iterate();
        ...
        /* Count the matches in the collected numbers. */
        return (0..n_nums)
            .map(|x| self.compare_low_bits(a_vals[x], b_vals[x]))
            .filter(|x| *x)
            .count();
    }
}
...
fn main() {
    println!(
        "Part 1 = {}\nPart 2 = {}\n",
        DualGenerator::new(634, 301).count_next_matches(40_000_000),
        DualGenerator::new(634, 301).non_seq_matches(5_000_000),
    )
}
